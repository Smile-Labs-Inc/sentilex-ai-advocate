    ...
    def synthesize(
        ...
        if validation_output.issues:
            ...
            for issue in validation_output.issues:
                emoji = "🔴" if issue.severity == "critical" else "⚠️" if issue.severity == "warning" else "ℹ️"
                response_parts.append(
...
def create_refusal_synthesizer() -> Runnable:
    ...
    def synthesize_refusal(validation_output: ValidationOutput) -> RefusalOutput:
        ...
        for issue in critical_issues:
            reason_parts.append(f"• {issue.description}\n")
...
def format_confidence_note(
    ...
    # Overall confidence
    if confidence >= 0.8:
        notes.append("✅ High confidence in this analysis.")
    elif confidence >= 0.5:
        notes.append("⚠️ Moderate confidence in this analysis.")
    else:
        notes.append("🔴 Low confidence in this analysis.")

    # Citation verification
    if all_citations_verified:
        notes.append(
            "✅ All citations have been verified against legal sources.")
    else:
        notes.append("⚠️ Some citations could not be verified.")

    # Hallucination detection
    if no_hallucination_detected:
        notes.append("✅ No external legal knowledge was used.")
    else:
        notes.append("🔴 Potential use of external legal knowledge detected.")
...
def generate_refusal_suggestions(issues) -> str:
    ...
    if "missing_sources" in issue_types:
        suggestions.append(
            "• Try rephrasing your question to be more specific about "
            "the legal topic or statute you're asking about."
        )

    if "hallucination" in issue_types:
        suggestions.append(
            "• The system detected content that may not be from legal sources. "
    ...
    if "missing_citation" in issue_types:
        suggestions.append(
            "• The analysis lacks proper citations. Try asking about a specific "
            "law or legal provision."
        )

    if not suggestions:
        suggestions.append(
            "• Please rephrase your question or consult with a legal professional."
        )
