...
- [ ] Design supervisor flow in LangGraph
- [ ] Implement routing logic to delegate to specialist agents
- [ ] Create prompt for supervisor to understand task categorization
- [ ] Build error recovery (if agents fail, retry with different approach)
- [ ] Implement reasoning trace logging for explainability

**3.2 - Researcher Agent**
- [ ] Build agent to analyze case facts and extract key information
- [ ] Create prompts for structured extraction (parties, allegations, timeline)
- [ ] Implement fact validation against RAG knowledge base
- [ ] Generate "Problem Category" recommendations (e.g., "Sexual Abuse under Penal Code 363")
- [ ] Output structured JSON with extracted facts

**3.3 - Legal Auditor Agent** (Specialized Compliance Check)
- [ ] Create auditor agent with single responsibility: audit Researcher's work
- [ ] Build prompts to check:
  - Is the categorization legally sound under Sri Lankan law?
  - Are there alternative legal interpretations?
  - Does the categorization comply with Online Safety Act 2021?
  - Are there jurisdictional concerns?
- [ ] Implement rejection with detailed reasoning (explain why category might be wrong)
- [ ] Create "confidence score" for each audit (0-100%)
- [ ] If confidence < 70%, escalate to human lawyer review
...
- [ ] Design message passing format between agents (JSON schema)
- [ ] Implement tool calling for agent-to-agent queries
- [ ] Create shared context manager (all agents access same facts)
- [ ] Build agent health monitoring (log execution time, success rate)
- [ ] Implement circuit breaker (if agent fails 3x, notify admin)

**3.6 - Reasoning & Explainability**
- [ ] Log all agent decisions with reasoning (why did Auditor reject this?)
- [ ] Create debug mode to visualize agent flow
- [ ] Generate "Decision Trail" PDF (show all agent interactions)
- [ ] Implement human-in-the-loop review UI (lawyer can override/approve)
...
- [ ] Convert PDFs to structured text with section references
- [ ] Create semantic chunking (split by legal concepts, not arbitrary lengths)

**4.2 - Vector Embedding & Retrieval**
- [ ] Use OpenAI embeddings (or open-source alternative like BGE)
- [ ] Create vector database (Pinecone, Weaviate, or Supabase pgvector)
- [ ] Implement semantic search for legal queries
- [ ] Build metadata filters (search by act, section, year, etc.)
- [ ] Create hybrid search (keyword + semantic) for accuracy

**4.3 - RAG Pipeline for Legal Queries**
- [ ] Build retrieval pipeline: `Query → Embedding → Vector Search → Reranking`
- [ ] Implement reranking using cross-encoder (e.g., `ms-marco-MiniLM-L-12-v2`)
- [ ] Create context window manager (include top 5-10 relevant sections)
- [ ] Implement citation tracking (every answer includes source section numbers)
- [ ] Build confidence scoring (low confidence → escalate to human lawyer)
